...
$sql = "SELECT tr.resultID, t.testName, tr.startTime, tr.endTime, tr.result, ts.systemName, tr.errorLog from tests t, testResults tr, systemsUnderTest ts WHERE t.testID=tr.testID and ts.sutID=tr.sutID";
$rs = $db ->Execute($sql);
...
if (!$rs) {
    ...
    while (!$rs->EOF) {
        ...
        echo '<td>'.$result.'</td>';
        $sut =  $rs->fields['systemName'];
        echo '<td>'.$sut.'</td>';
        $resultID =  $rs->fields['resultID'];
        $durationSQL = "select (MICROSECOND(endTime)-MICROSECOND(startTime)) from testResults where resultID= ".$resultID;
        $result=$db ->Execute($durationSQL);
        $row=$result->FetchRow();
        $duration=$row[0];
        echo '<td>'.$duration.' milliseconds</td>';
        $errorLog =  $rs->fields['errorLog'];
        echo '<td>'.$errorLog.'</td>';
        echo '</tr>';
        $rs->MoveNext();
    }
